Ultrathink on a batch evaluation runner file named "batch_eval_runner.py" which generates evaluation results that captures the specific voice agent being run. The different voice agent is passed in as a command line parameter for "eval_runner.py" such as:

"uv run python eval_runner.py --bots bot0.py bot1.py bot2.py"
...
Each agent is invoked using "uv run bot.py" where bot.py is any specific implementation of the bot. 
